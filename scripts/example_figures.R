...
A<- data.frame(N = c(t(hold6[[1]][,501:1000,100])), species = rep(1:100, each = 500), time = 1:500) %>%
  filter(N >0) %>% 
  group_by(time) %>% 
  summarise(N = sum(N)) %>% 
...
B<- data.frame(N = c(t(hold6[[1]][,501:1000,100])), species = rep(1:100, each = 500), time = 1:500) %>%
  filter(N >0) %>% 
  group_by(time) %>% 
  summarise(S = sum(N>0)) %>% 
...
A<- data.frame(N = c(hold[[1]][,,100]), species = rep(1:100, each = 80), patch = 1:80) %>%
  filter(N >0) %>% 
  group_by(patch) %>% 
  summarise(N = sum(N)) %>% 
  ggplot(aes(x = patch, y = N))+
  geom_point()+
  scale_size_continuous(range = c(0.1,1), guide = F)+
  ggtitle("gamma = 2")

B<- data.frame(N = c(hold[[1]][,,100]), species = rep(1:100, each = 80), patch = 1:80) %>%
  filter(N >0) %>% 
  group_by(patch) %>% 
  summarise(S = sum(N>0)) %>% 
  ggplot(aes(x = patch, y = S))+
  geom_point()+
  scale_size_continuous(range = c(0.1,1), guide = F)+
  ggtitle("gamma = 2")
